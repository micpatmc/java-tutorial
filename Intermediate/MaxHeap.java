...
public class MaxHeap {
     ...
     * Java doesn't have an actual "Max-Heap" interface, so a "PriorityQueue" is
     * used instead
     * 
     * -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
     * =-=-=-=-=-=-=-=
     ...
     * ----------------
     * 
     * Now, the number "42" is at the top of the Max-Heap because it's larger than
     * "12"
     * and the Max-Heap is ordered so larger values are at the top and smaller on
     * the bottom
     * 
     * -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
     * =-=-=-=-=-=-=-=
     ...
     * =-=-=-=-=-=-=-=
     * 
     * Another function we are able to do is "poll" the number from the Max-Heap,
     * which will ALWAYS
     ...
     * ----------------
     * 
     * "num" is now equal to "42" because that was the number at the top of the
     * Max-Heap
     * 
     * -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
     * =-=-=-=-=-=-=-=
     ...
     * =-=-=-=-=-=-=-=
     * 
     * A function that Max-Heaps use that is also important is maxHeap.peek() to
     * look at whatever
     ...
     * ----------------
     * 
     * "num" is equal to "12", but "12" is still in the Max-Heap because it was only
     * read and
     ...
     * =-=-=-=-=-=-=-=
     * 
     * Lastly, an important function for Max-Heap is maxHeap.isEmpty() which will
     * check if the Queue
     * is empty or not
     * 
